...
jQuery(document).ready(function () {
    ...
    $j('#btn-get-mockup').on('click',function () {
        $j('#btn-get-mockup').css('display','none');
        $j('#tb-loading-mockup').css('display','block');
        ...
        var url = getTempate_url + '?brand_id='+brand_id+'&model_id='+model_id;
        $j.ajax({
            url: url,
            method: "GET",
            success: function (result) {
                if(result.status == 1){
                    ...
                    $j('#btn-get-mockup').css('display','block');
                    $j('#tb-loading-mockup').css('display','none');

                    var items = result.items;
                    var result_html = '';
                    for(var i = 0; i  < items.length; i++){
                        var item = items[i];
                        var is_recommend = 'Không';
                        if(item['is_recommend'] == 1){
                            is_recommend = 'Có';
                        }
                        var link_download = base_download_url + '?mockup_file=' + item['mockup_file'];
                        result_html = result_html +'<tr>';
                        result_html = result_html + '<td>'+brand_name+'</td>';
                        result_html = result_html + '<td>'+model_name+'</td>';
                        result_html = result_html + '<td>'+item['upload_by']+'</td>';
                        result_html = result_html + '<td>'+is_recommend+'</td>';
                        result_html = result_html + '<td><a href="'+link_download+'">Download</a></td>';
                        result_html = result_html + '</tr>';
                    }
